...
机器人加入群聊必须由目标群管理员明确授权。管理员通过机器人用户名或 `potato.im/<bot_username>` 链接打开机器人资料，并在 Potato 客户端中完成添加。

机器人服务不得保存或使用个人账号凭据，也不得通过个人账号客户端协议模拟加群。

## 本系统的多机器人管理接口

以下接口都要求已登录的超级管理员身份，使用系统现有的 Bearer Token 鉴权。
